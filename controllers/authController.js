...
exports.login = catchAsync(async (req, res, next) => {
    ...
    // 2) Chech if user exist and password correct
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(new AppError('You are not logged. Please log in to get acces', 401));
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new AppError('Token is invalid or has ecpired', 400));
        user.password = req.bode.password;
        user.passwordConfirm = req.bode.passwordConfirm;
